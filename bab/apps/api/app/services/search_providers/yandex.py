"""Yandex reverse image search provider adapter."""
...
class YandexProviderAdapter(BaseProviderAdapter):
    """Adapter for Yandex-compatible reverse image search APIs."""
    
    async def test_connection(self) -> Dict[str, Any]:
        if not self.api_key:
            return {
                "status": "invalid_credentials",
                "message": "Yandex API key not configured. Search provider not configured.",
            }
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if response.status_code == 200:
                    return {
                        "status": "connected",
                        "message": "Yandex search API connected",
    ...
    async def search_by_image(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.provider.timeout_ms / 1000) as client:
                ...
                if response.status_code == 200:
                    data = response.json()
                    results = data.get("sites", data.get("results", []))
